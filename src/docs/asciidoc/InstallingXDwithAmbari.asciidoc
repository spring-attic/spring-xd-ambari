...
IMPORTANT: We currently have a Release Candidate build available. Please try it and report any issues using the link:https://github.com/spring-projects/spring-xd-ambari/issues[GitHub issue tracking for the project].
...
Download the Spring XD Ambari plugin from the Spring Repository (link:http://repo.spring.io/libs-milestone-local/org/springframework/xd/spring-xd/1.2.0.RC1/SPRINGXD-1.2-1.tar[http://repo.spring.io/libs-milestone-local/org/springframework/xd/spring-xd/1.2.0.RC1/SPRINGXD-1.2-1.tar]) and copy it to the YUM repository machine for your Ambari installation. We're copying the file to a the `/staging` directory.
