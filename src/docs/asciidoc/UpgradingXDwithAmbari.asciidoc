...
Look under `Installed Packages`, if the `spring-xd-plugin` is listed as version 1.2-2 then no further action is needed. The datafiles are now stored in `/var/run/springxd/data/jobs`.
