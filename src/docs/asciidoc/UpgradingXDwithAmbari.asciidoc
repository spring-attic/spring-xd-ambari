...
Look under `Installed Packages`, if the `spring-xd-plugin` is listed as version 1.2-2 then no further action is needed. The the datafiles are now stored in `/var/run/springxd/data/jobs`.
...
For and upgrade from Spring XD 1.2.1 execute this command to copy the files:
...
For and upgrade from Spring XD 1.2.0 execute this command to copy the files:
